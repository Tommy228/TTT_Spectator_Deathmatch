	...
	self.OldLogs.UpdateDates = function()
		...
		for _, v in pairs({older, latest}) do
			for k, data in pairs(v) do
				v[k] = tonumber(data) -- TODO: changed it... Test!
			end
		end
		...
		for y = 0, years do
			...
			if Damagelog.OldLogsDays[year] then
                ...
                for m = start_range, end_range do
                    if Damagelog.OldLogsDays[year][m] then
	...
	self.DateChoice.OnNodeSelected = function(panel, selected)
		if selected.rounds then
            ...
            for _, v in ipairs(selected.rounds) do
				if v.date ~= lunix then -- Hack way to stupid duplicate rounds
