...
function Damagelog:DrawDamageTab(x, y)
	...
	self.RoundPanel.Paint = function() 
	...
	self.Filters.DoClick = function(Filters, forcedX, forcedY)
		...
		for k in pairs(Damagelog.filters) do
	...
	self.PlayerSelect.UpdatePlayers = function(self)
		...
		if #Damagelog.Highlighted > 0 then
			...
			for _, v in ipairs(Damagelog.Highlighted) do
	...
	self.DamageInfo:AddColumn(TTTLogTranslate(GetDMGLogLang, "DmgInfo")).DoClick = function() 
	...
	if PlayedRounds > 1 or client:CanUseDamagelog() and PlayedRounds > 0 then
		local i_count = 1
		...
		if not client:CanUseDamagelog() then
			PlayedRounds = PlayedRounds - 1
		end

		for i = i_count, PlayedRounds do
			if i == PlayedRounds and client:CanUseDamagelog() then
		...
		if not client:CanUseDamagelog() or (GetConVar("ttt_dmglogs_currentround"):GetBool() or not client:IsActive()) then
	...
	self.Round.OpenMenu = function(self, pControlOpener)
		...
		for _, v in pairs(sorted, "data") do
...
net.Receive("DL_RefreshDamagelog", function()
	...
	if not IsValid(client) then return end -- sometimes happens while joining
    
	if not client.CanUseDamagelog then return end
